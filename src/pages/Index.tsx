  ...
  const refreshGameCode = useCallback(async (sid: string) => {
    try {
      ...
      const result = await supabase
        // @ts-ignore
        .from("game_sessions")
        .select("html_code, css_code, js_code, status, chat_history")
        .eq("id", sid)
        .maybeSingle();
    ...
    try {
      ...
      if (!sessionId) {
        ...
        setChatHistory([
          ...
          { role: "assistant", content: initData.gamePlan.chat_response },
        ]);
        ...
        // Execute tool calls
        const { error: execError } = await supabase.functions.invoke("execute-tool-calls", {
          body: {
            sessionId: newSessionId,
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Iterative game development powered by AI
            </p>
          </div>
          {status !== "initial" && (
            <div className="px-3 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-sm">
              {status.replace(/_/g, " ")}
            </div>
          )}
        </div>
